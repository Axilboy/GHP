...
import { GameShell, TimerBadge } from '../engine/GameShell';
...
export const bunkerContentPackOptions = [
  ...
  { id: 'party', name: 'Вечеринка в бункере', badge: 'пак' },
  { id: 'drinks', name: 'Бар после конца света', badge: '18+' },
  { id: 'couples', name: 'Пары и доверие', badge: 'пак' },
  { id: 'adult_couples', name: '18+ для пар', badge: '18+' },
  ...
  { id: 'fandom', name: 'Фан-вселенные', badge: 'пак' },
  { id: 'harry_potter', name: 'Гарри Поттер', badge: 'пак' },
  { id: 'lotr', name: 'Властелин колец', badge: 'пак' },
  { id: 'retro_movies', name: 'Ретро-кино', badge: 'пак' },
  { id: 'video_games', name: 'Видеоигры', badge: 'пак' },
];
...
export function BunkerReveal({ room, card, revealRole, action, navigate, error }) {
  ...
  return <GameShell gameId="bunker" room={room} navigate={navigate} gameTitle="Бункер" right={<span className="badge">Карточка</span>}><section className="role role-reveal-screen bunker-reveal"><span className="eyebrow">Личная карточка</span><h1>{card ? card.profession : 'Кто вы после катастрофы?'}</h1><p>{card ? 'Нажимайте на поля карточки: открытые можно показать на столе, скрытые останутся закрытыми.' : 'Откройте карточку так, чтобы её видели только вы.'}</p>{card ? <BunkerCard card={card} visibleFields={visibleFields} onToggle={toggleField} interactive /> : <button className="secret-role-card bunker-secret-card" onClick={revealRole}><span className="role-card-inner"><span className="role-card-face role-card-back"><b>?</b><small>Открыть персонажа</small><em>Покажите экран только себе</em></span></span></button>}{card && <><p className="bunker-table-note">Открытые поля видны на этом телефоне. Можно показать экран компании или положить телефон на стол.</p><button className="button primary full" onClick={() => action('role_seen')}>Готов к обсуждению</button></>}{error && <ErrorText text={error} />}<p className="progress-note">Карточки открыли: {room.round.seenCount} из {bunkerContestants(room).length} игроков</p></section></GameShell>;
}
...
export function BunkerVoting({ room, me, action, navigate, now, error }) {
  ...
  return <GameShell gameId="bunker" room={room} navigate={navigate} gameTitle="Бункер" right={<TimerBadge endsAt={room.round.votingEndsAt} now={now} />}><section className="vote wrap bunker-vote"><span className="eyebrow">{room.round.voteRound > 1 ? 'Переголосование' : 'Совет убежища'}</span><h1 className="play-title">Кто не проходит в бункер?</h1><p>Голосуйте за игрока, чья карточка кажется компании менее полезной для выживания.</p>{canVote ? <div className="vote-list">{candidates.map((player) => <button className={room.round.myVote === player.id ? 'selected' : ''} key={player.id} onClick={() => action('vote', { targetId: player.id })}>{player.name}</button>)}</div> : <p className="waiting-host">Вы уже вне отбора и ждёте решения остальных.</p>}<p>Проголосовали: {room.round.votesCount} из {bunkerContestants(room).length} претендентов</p>{error && <ErrorText text={error} />}</section></GameShell>;
}
...
export function BunkerHostSettings({ room, catalog = {}, profile, update, changeGame, themeSettings }) {
  ...
  const selectedPacks = room.settings.contentPackIds || ['classic'];
  const togglePack = (id) => {
    const next = selectedPacks.includes(id) ? selectedPacks.filter((item) => item !== id) : [...selectedPacks, id];
    if (next.length) update({ contentPackIds: next });
  };
  return <section className="section settings alias-host-panel bunker-host-panel"><h2>Бункер</h2><GameSwitcher gameId={room.gameId} changeGame={changeGame} truthDareLocked={!hasProAccess(profile)} /><details className="settings-group" open><summary><span>Настройки раунда</span><small>{room.settings.roundSeconds / 60} мин · {room.settings.votingSeconds} сек</small></summary><Setting label="Режим вскрытия"><select value={revealMode} onChange={(event) => update({ revealMode: event.target.value })}><option value="private_table">Личная карточка на стол</option><option value="public_turns">Вскрытие по очереди</option></select></Setting><div className="dictionary-checklist">{packs.map((pack) => { const locked = !hasThemedContentAccess(profile, 'bunker', pack); return <label key={pack.id} className={locked ? 'locked' : ''}><input type="checkbox" checked={selectedPacks.includes(pack.id)} disabled={locked} onChange={() => togglePack(pack.id)} /><span><b>{pack.name}</b><small>{locked ? 'пропуск или Bunker Pass' : pack.badge || (pack.tier === 'free' ? 'включён' : 'открыт')}</small></span></label>; })}</div><Setting label="Обсуждение"><select value={room.settings.roundSeconds} onChange={(event) => update({ roundSeconds: Number(event.target.value) })}><option value="180">3 минуты</option><option value="300">5 минут</option><option value="420">7 минут</option></select></Setting><Setting label="Голосование"><select value={room.settings.votingSeconds} onChange={(event) => update({ votingSeconds: Number(event.target.value) })}><option value="30">30 секунд</option><option value="45">45 секунд</option><option value="60">60 секунд</option></select></Setting><p className="settings-note">{revealMode === 'public_turns' ? 'Игроки по очереди вскрывают выбранные поля. Все видят, что раскрыто, а что оставлено скрытым.' : 'Игрок решает, какие поля показать компании со своего телефона.'}</p>{!hasAccess && <p className="settings-note">Тематический пропуск откроет свою тему, а Bunker Pass, WeekendPass или PRO откроет все сценарии Бункера.</p>}{!hasProAccess(profile) && <p className="settings-note">Создание комнаты для «Правды или действия» входит в PRO.</p>}</details>{themeSettings}</section>;
}
