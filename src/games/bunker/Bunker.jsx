...
export function BunkerHostSettings({ room, catalog = {}, profile, update, changeGame, themeSettings }) {
  ...
  return <section className="section settings alias-host-panel bunker-host-panel"><h2>Бункер</h2><GameSwitcher gameId={room.gameId} changeGame={changeGame} truthDareLocked={!hasProAccess(profile)} /><details className="settings-group" open><summary><span>Настройки раунда</span><small>{room.settings.roundSeconds / 60} мин · {room.settings.votingSeconds} сек</small></summary><Setting label="Режим вскрытия"><select value={revealMode} onChange={(event) => update({ revealMode: event.target.value })}><option value="private_table">Личная карточка на стол</option><option value="public_turns">Вскрытие по очереди</option></select></Setting><Setting label="Набор событий"><select value={room.settings.contentPackId || 'classic'} onChange={(event) => update({ contentPackId: event.target.value })}>{packs.map((pack) => { const locked = !hasThemedContentAccess(profile, 'bunker', pack); return <option key={pack.id} value={pack.id} disabled={locked}>{pack.name} · {locked ? 'пропуск или Bunker Pass' : pack.badge || (pack.tier === 'free' ? 'включён' : 'открыт')}</option>; })}</select></Setting><Setting label="Обсуждение"><select value={room.settings.roundSeconds} onChange={(event) => update({ roundSeconds: Number(event.target.value) })}><option value="180">3 минуты</option><option value="300">5 минут</option><option value="420">7 минут</option></select></Setting><Setting label="Голосование"><select value={room.settings.votingSeconds} onChange={(event) => update({ votingSeconds: Number(event.target.value) })}><option value="30">30 секунд</option><option value="45">45 секунд</option><option value="60">60 секунд</option></select></Setting><p className="settings-note">{revealMode === 'public_turns' ? 'Игроки по очереди вскрывают выбранные поля. Все видят, что раскрыто, а что оставлено скрытым.' : 'Игрок решает, какие поля показать компании со своего телефона.'}</p>{!hasAccess && <p className="settings-note">Тематический пропуск откроет свою тему, а Bunker Pass, WeekendPass или PRO откроет все сценарии Бункера.</p>}{!hasProAccess(profile) && <p className="settings-note">Создание комнаты для «Правды или действия» входит в PRO.</p>}</details>{themeSettings}</section>;
}
