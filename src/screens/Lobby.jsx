...
import { AdBreakModal, Counter, ErrorText, GameSwitcher, LobbyPlayerAdBanner, NameModal, Setting } from '../shared/ui';
import { getOwnedThemeSet, getRoomThemeName, hasAnyThemePass, hasProAccess, hasTimedGameAccess, roomThemesFallback } from '../shared/helpers';
...
import { RulesModal, SpyModeTip, dictionaryCount, spySubjectCopy } from '../games/spy/Spy';
import { TruthDareHostSettings, TruthDareLobbySummary, TruthDareRulesModal, TruthDareScoreLine } from '../games/truthdare/TruthDare';
...
export function Lobby({ room, me, isHost, catalog, profile, action, leave, navigate, error }) {
  ...
  const rulesSeenKey = `gamehubparty_${room.gameId || 'spy'}_rules_seen`;
  const [rulesOpen, setRulesOpen] = useState(() => localStore.getItem(rulesSeenKey) !== 'yes');
  ...
  useEffect(() => {
    setRulesOpen(localStore.getItem(rulesSeenKey) !== 'yes');
  }, [rulesSeenKey]);
  const onlinePlayers = room.players.filter((player) => player.online);
  const gameTitle = room.gameId === 'alias' ? 'Alias' : room.gameId === 'bunker' ? 'Бункер' : room.gameId === 'truthdare' ? 'Правда или действие' : 'Шпион';
  const minPlayers = room.gameId === 'truthdare' ? 2 : room.gameId === 'alias' || room.gameId === 'bunker' ? 4 : 3;
  const canStart = onlinePlayers.length >= minPlayers && onlinePlayers.every((player) => player.ready);
  const closeRules = () => {
    localStore.setItem(rulesSeenKey, 'yes');
    setRulesOpen(false);
  };
  const startRound = () => {
    if (room.adPolicy?.enabled) {
      const slot = room.gameId === 'alias' ? 'alias_pre_round_interstitial' : room.gameId === 'bunker' ? 'bunker_pre_round_interstitial' : room.gameId === 'truthdare' ? 'truthdare_pre_round_interstitial' : 'spy_pre_round_interstitial';
  ...
  const notReady = onlinePlayers.filter((player) => !player.ready);
  const waitingLabel = notReady.length <= 2
    ? `Ждём: ${notReady.map((player) => player.name).join(', ')}`
    : `Ждём: ${notReady.slice(0, 2).map((player) => player.name).join(', ')} +${notReady.length - 2}`;
  const hasScore = Object.values(room.scores || {}).some((value) => value > 0);
  return <main className={`room-theme-${room.themeId || 'ghp'}`}><Header navigate={navigate} gameTitle={gameTitle} brandTheme={room.themeId || 'ghp'} right={<span className="badge">{isHost ? 'Хост' : 'Игрок'}</span>} /><div className="lobby wrap"><section className="invite"><span>Код комнаты</span><strong>{room.code.slice(0, 3)} {room.code.slice(3)}</strong><div className="inline"><button className="button small secondary" onClick={() => navigator.clipboard?.writeText(inviteUrl)}>Скопировать ссылку</button><button className="button small secondary" onClick={() => navigator.share?.({ title: `Игра ${gameTitle}`, url: inviteUrl })}>Поделиться</button></div>{qr && <details><summary>Показать QR-код</summary><img className="qr-image" src={qr} alt="QR-код комнаты" /></details>}</section><section className="section lobby-players"><div className="section-title"><h2>Игроки</h2><span>{readyCount}/{onlinePlayers.length} готовы</span></div><button className="my-profile" onClick={() => setEditingName(true)}><i>{me.name[0]?.toUpperCase()}</i><span><b>{me.name}</b><small>Изменить имя</small></span></button><ul className="players">{room.players.filter((player) => player.id !== me.id).map((player) => <li key={player.id}><i>{player.name[0]?.toUpperCase()}</i><span>{player.name}{player.id === room.hostId ? ' · хост' : ''}</span><small className={player.ready ? 'ready' : ''}>{!player.online ? 'Не в сети' : player.ready ? 'Готов' : 'Не готов'}</small></li>)}</ul></section>{isHost ? <HostSettings room={room} catalog={catalog || {}} dictionaries={catalog?.dictionaries || []} themes={catalog?.themes || roomThemesFallback} profile={profile} action={action} /> : <LobbyGameSummary room={room} catalog={catalog || {}} dictionaries={catalog?.dictionaries || []} themes={catalog?.themes || roomThemesFallback} profile={profile} action={action} />}<LobbyPlayerAdBanner adPolicy={room.adPolicy} navigate={navigate} />{hasScore && (room.gameId === 'alias' ? <AliasScoreLine scores={room.scores} /> : room.gameId === 'bunker' ? <BunkerScoreLine room={room} /> : room.gameId === 'truthdare' ? <TruthDareScoreLine room={room} /> : <div className="score-line"><span>Мирные: {room.scores.civilians}</span><span>Шпионы: {room.scores.spies}</span></div>)}{error && <ErrorText text={error} />}<div className="lobby-links"><button className="link" onClick={() => setRulesOpen(true)}>Как играть</button><button className="link danger leave" onClick={leave}>Выйти из комнаты</button></div>{isHost ? <button className="button primary full sticky" disabled={!canStart} onClick={startRound}>{canStart ? 'Начать раунд' : onlinePlayers.length < minPlayers ? `Нужно минимум ${minPlayers} игрока онлайн` : waitingLabel}</button> : <button className={`button full sticky ${me.ready ? 'secondary' : 'primary'}`} onClick={() => action('set_ready', { ready: !me.ready })}>{me.ready ? 'Я не готов' : 'Я готов'}</button>}</div>{editingName && <NameModal value={nextName} setValue={setNextName} close={() => setEditingName(false)} save={async () => { const result = await action('rename_player', { name: nextName }); saveGuestDisplayName(result.name); setNextName(result.name); setEditingName(false); }} />}{rulesOpen && (room.gameId === 'alias' ? <AliasRulesModal close={closeRules} /> : room.gameId === 'bunker' ? <BunkerRulesModal close={closeRules} /> : room.gameId === 'truthdare' ? <TruthDareRulesModal close={closeRules} /> : <RulesModal close={closeRules} />)}{adBreak && <AdBreakModal {...adBreak} close={() => setAdBreak(null)} />}</main>;
}
...
export function HostSettings({ room, catalog = {}, dictionaries, themes = roomThemesFallback, profile, action }) {
  ...
  if (room.gameId === 'truthdare') return <TruthDareHostSettings room={room} catalog={catalog} profile={profile} update={update} changeGame={changeGame} themeSettings={themeSettings} />;
  const fullSpyAccess = hasTimedGameAccess(profile, 'spy');
  const owned = new Set(dictionaries
    .filter((dictionary) => fullSpyAccess || dictionary.free || (profile?.ownedDictionaryIds || ['base']).includes(dictionary.id) || hasAnyThemePass(profile, dictionary.themeIds))
  ...
  return <section className="section settings host-control-panel"><h2>Шпион</h2><GameSwitcher gameId={room.gameId || 'spy'} changeGame={changeGame} truthDareLocked={!hasProAccess(profile)} />{!hasProAccess(profile) && <p className="settings-note">Создание комнаты для «Правды или действия» входит в PRO.</p>}<details className="settings-group" open><summary><span>Настройки раунда</span><small>{spyCountLabel(room.settings.spyCount)} · {room.settings.roundSeconds / 60} мин</small></summary><AdvancedSpySettings room={room} action={action} profile={profile} /><SpyModeTip subjectType={subjectType} /><Setting label="Время раунда"><select value={room.settings.roundSeconds} onChange={(event) => update({ roundSeconds: Number(event.target.value) })}><option value="300">5 минут</option><option value="480">8 минут</option><option value="600">10 минут</option></select></Setting><Setting label="Голосование"><select value={room.settings.votingSeconds} onChange={(event) => update({ votingSeconds: Number(event.target.value) })}><option value="20">20 секунд</option><option value="30">30 секунд</option><option value="45">45 секунд</option><option value="60">60 секунд</option></select></Setting><Setting label="Очков для победы"><Counter value={room.settings.targetScore} min={3} max={10} change={(targetScore) => update({ targetScore })} /></Setting></details><details className="settings-group"><summary><span>{copy.dictionaryTitle}</span><small>{room.settings.dictionaryIds.length} выбрано</small></summary><div className="dictionary-checklist">{visibleDictionaries.map((dictionary) => <label key={dictionary.id} className={!owned.has(dictionary.id) ? 'locked' : ''}><input type="checkbox" checked={room.settings.dictionaryIds.includes(dictionary.id)} disabled={!owned.has(dictionary.id)} onChange={() => toggleDictionary(dictionary.id)} /><span><b>{dictionary.name}</b><small>{dictionaryCount(dictionary)} · {owned.has(dictionary.id) ? 'доступен' : 'в магазине'}</small></span></label>)}</div>{subjectType === 'location' && profile?.customLocations?.length > 0 && <p className="settings-note">Ваши собственные локации тоже попадут в раунд автоматически.</p>}</details>{themeSettings}</section>;
}
...
export function LobbyGameSummary({ room, dictionaries, themes = roomThemesFallback, profile, action }) {
  ...
  if (room.gameId === 'truthdare') return <TruthDareLobbySummary room={room} themeSummary={themeSummary} />;
  const subjectType = room.settings.subjectType || 'location';
  const copy = spySubjectCopy(subjectType);
  ...
  return <section className="lobby-game-summary"><span>Выбрана игра</span><h2>Шпион</h2><p>{copy.typeValue} · {room.settings.roundSeconds / 60} минут · {spyCountLabel(room.settings.spyCount)}</p><small>{copy.dictionaryTitle}: {names || copy.defaultDictionary}</small>{themeSummary}</section>;
}

function spyCountLabel(count = 1) {
  const value = Math.max(1, Number(count) || 1);
  if (value === 1) return '1 шпион';
  if (value >= 2 && value <= 4) return `${value} шпиона`;
  return `${value} шпионов`;
}
