...
import { APP_DISPLAY_VERSION, APP_RELEASE_DATE, APP_RELEASE_NAME } from '../version';
...
export function DemoPage({ navigate }) {
  ...
  return <main className="app-screen demo-page">
    ...
    <section className="legal-hero wrap">
      ...
      <h1>GameHubParty v{APP_DISPLAY_VERSION}</h1>
    ...
    <section className="section wrap demo-status">
      ...
      <div className="purchase-status-grid"><article><b>{status?.activeRooms ?? '...'}</b><span>активных комнат</span></article><article><b>{status?.games?.length ?? '...'}</b><span>игры в каталоге</span></article><article><b>v{APP_DISPLAY_VERSION}</b><span>текущая версия</span></article><article><b>{updatedAt}</b><span>время открытия</span></article></div>
    </section>
    <section className="section wrap">
      <h2>Быстрый просмотр</h2>
      <div className="demo-links"><button onClick={() => navigate('home')}>Главная</button><button onClick={() => navigate('spy')}>Лендинг Шпиона</button><button onClick={() => navigate('store')}>Магазин</button><button onClick={() => navigate('profile')}>Профиль</button><button onClick={() => navigate('contacts')}>Контакты</button><button onClick={() => navigate('alias')}>Alias</button><button onClick={() => navigate('vk')}>VK-страница</button></div>
    </section>
    <section className="section wrap">
      ...
      <div className="screenshot-folders">{screenshotFolders.map((folder) => <article className="screenshot-folder-card" key={folder.title}>
        <div className="screenshot-folder-head"><div><b>{folder.title}</b><p>{folder.description}</p></div>{folder.shots.length > 0 && <span>{folder.shots.length} скринов</span>}</div>
        {folder.shots.length > 0 && <div className="screenshot-carousel">{folder.shots.map((shot) => <a key={shot.file} href={shot.file} target="_blank" rel="noreferrer"><img src={shot.file} alt={shot.title} loading="lazy" /><small>{shot.title}</small></a>)}</div>}
      </article>)}</div>
